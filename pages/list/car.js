...
Page({
  ...
  onShow: function () {
    ...
    for(let v of list) {
      v.checked = true;
      totalPrice += parseFloat(parseFloat(v.shopGoodsEntity.price) * parseFloat(v.amount || 1))
      list1.push(v)
    }
    this.setData({
      cartData:list,
  ...
  //结算
  toApply: function () {
    wx.setStorageSync('carList', this.data.selectArr);
    wx.navigateTo({
      url: '/pages/list/info?price='+this.data.totalPrice,
    })
  },
  /**
   * 生命周期函数--监听页面隐藏
